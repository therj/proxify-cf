...
async function getRoutes(db) {
  const { results } = await db.prepare("SELECT * FROM routes ORDER BY created_at DESC").all();
  return results;
}
...
async function createRoute(db, data) {
  ...
  const { results } = await db.prepare("SELECT * FROM routes WHERE id = ?1").bind(id).all();
  return results[0];
}
...
async function getRouteHeaders(db, route_id) {
  const { results } = await db.prepare("SELECT * FROM route_headers WHERE route_id = ?1").bind(route_id).all();
  return results.map((r) => ({ ...r, id: `${r.route_id}:${r.header_name}` }));
}
__name(getRouteHeaders, "getRouteHeaders");
async function addRouteHeader(db, route_id, header_name, header_value) {
  await db.prepare(
...
__name(addRouteHeader, "addRouteHeader");
async function removeRouteHeader(db, header_id) {
...
__name(removeRouteHeader, "removeRouteHeader");
...
async function createClient(db, data) {
  const id = crypto.randomUUID();
  const created_at = Date.now();
  ...
  const { results } = await db.prepare("SELECT * FROM clients WHERE id = ?1").bind(id).all();
  return results[0];
}
__name(createClient, "createClient");
async function updateClient(db, id, data) {
  const updates = [];
  const values = [];
  let paramIndex = 1;
  for (const [key, value] of Object.entries(data)) {
...
// ../../node_modules/.pnpm/jose@6.2.3/node_modules/jose/dist/webapi/lib/key_to_jwk.js
...
// ../../node_modules/.pnpm/jose@6.2.3/node_modules/jose/dist/webapi/key/export.js
...
// ../../node_modules/.pnpm/jose@6.2.3/node_modules/jose/dist/webapi/key/generate_key_pair.js
...
async function encryptPrivateJwk(jwkString, kekStr) {
...
__name(encryptPrivateJwk, "encryptPrivateJwk");
async function decryptPrivateJwk(encryptedPayload, kekStr) {
  let parsed;
  try {
...
__name(decryptPrivateJwk, "decryptPrivateJwk");
async function getKeys(db) {
  const res = await db.prepare("SELECT * FROM keys ORDER BY created_at DESC").all();
  const rows = res.results;
  return Array.isArray(rows) ? rows : [];
}
__name(getKeys, "getKeys");
async function createKey(db, data, kek) {
  ...
  const { results } = await db.prepare("SELECT * FROM keys WHERE kid = ?1").bind(kid).all();
  return {
    key: results[0],
    // Only return private key if it's client signed so they can download it
    privateJwk: data.mode === "client_signed" ? privateJwk : void 0
  };
}
__name(createKey, "createKey");
async function revokeKey(db, kid) {
...
async function getIssuedTokens(db) {
  const res = await db.prepare("SELECT * FROM issued_tokens ORDER BY issued_at DESC").all();
  const rows = res.results;
  return Array.isArray(rows) ? rows : [];
}
__name(getIssuedTokens, "getIssuedTokens");
async function mintToken(db, kid, client_id, label, expires_in_days = 30, kek) {
  ...
  const decryptedJwkStr = await decryptPrivateJwk(results[0].private_jwk_encrypted, kek);
  const privateJwk = JSON.parse(decryptedJwkStr);
  const privateKey = await importJWK(privateJwk, "ES256");
  const jti = crypto.randomUUID();
  const issued_at = Date.now();
  const expires_at = issued_at + expires_in_days * 24 * 60 * 60 * 1e3;
  const token = await new SignJWT({ jti, client_id }).setProtectedHeader({ alg: "ES256", kid }).setIssuedAt(Math.floor(issued_at / 1e3)).setExpirationTime(Math.floor(expires_at / 1e3)).sign(privateKey);
  await db.prepare(
  ...
  const { results: tokenResults } = await db.prepare("SELECT * FROM issued_tokens WHERE jti = ?1").bind(jti).all();
...
// src/repo/grants.ts
async function getGrants(db) {
  const { results } = await db.prepare("SELECT * FROM client_route_grants ORDER BY granted_at DESC").all();
  return results;
}
__name(getGrants, "getGrants");
async function createGrant(db, client_id, route_id) {
  ...
  ).bind(client_id, route_id).all();
  return results[0];
}
__name(createGrant, "createGrant");
async function revokeGrant(db, client_id, route_id) {
...
async function getAuditLogs(db) {
  const { results } = await db.prepare("SELECT * FROM audit_log ORDER BY ts DESC LIMIT 100").all();
  return results;
}
__name(getAuditLogs, "getAuditLogs");
async function appendAudit(db, actor, action, target, meta) {
  const id = crypto.randomUUID();
  const ts = Date.now();
  await db.prepare(
...
__name(appendAudit, "appendAudit");
...
var proxyHandler = /* @__PURE__ */ __name(async (c) => {
...
var serveAdminAssets = /* @__PURE__ */ __name((c) => {
...
// ../../node_modules/.pnpm/wrangler@4.86.0_@cloudflare+workers-types@4.20260430.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@4.86.0_@cloudflare+workers-types@4.20260430.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-2rP1qP/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@4.86.0_@cloudflare+workers-types@4.20260430.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-2rP1qP/middleware-loader.entry.ts
