...
export async function getKeys(db: D1Database): Promise<Key[]> {
  const res = await db.prepare('SELECT * FROM keys ORDER BY created_at DESC').all<Key>();
...
export async function getIssuedTokens(db: D1Database): Promise<IssuedToken[]> {
  const res = await db.prepare('SELECT * FROM issued_tokens ORDER BY issued_at DESC').all<IssuedToken>();
  const rows = res.results;
  return Array.isArray(rows) ? rows : [];
}
